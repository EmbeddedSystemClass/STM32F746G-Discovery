...
static void _UserDraw(WM_HWIN hWin, int Stage) {
	...
	case GRAPH_DRAW_FIRST:
		...
		GUI_DrawHLine(y,0,472);
		//GUI_POINT p1[3]={{0,0},{ArrowSize/2,ArrowSize/2},{0,ArrowSize}};//>
		GUI_POINT p2[3]={{0,ArrowSize/2},{ArrowSize/2,0},{ArrowSize/2,ArrowSize}};//<
		GUI_FillPolygon(p2, 3,6, y-ArrowSize/2);
		break;
	case GRAPH_DRAW_LAST:
		for(int i=0;i<FFT_LENGTH;i++){
			GUI_SetColor(GUI_BLUE);
			//GUI_DrawVLine(i,120-appGlobals.fftDisplay[i],120);
		}
		break;
	}
}
...
void GUI_Task(void const *arg) {
	...
	//
	WM_HWIN hItem = WM_GetDialogItem(WM_HBKWIN, GUI_ID_GRAPH1);
